...
export class StatusApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async apiStatusGetBlockInfoProviderStatusGet(provider?: string, network?: string, includeSidechains?: boolean, _options?: Configuration): Promise<RequestContext> {
...
export class StatusApiResponseProcessor {
     ...
     */
    public async apiStatusGetBlockInfoProviderStatusGet(response: ResponseContext): Promise<{ [key: string]: BlockInfoProviderStatusResult; }> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: { [key: string]: BlockInfoProviderStatusResult; } = ObjectSerializer.deserialize(
