...
export default class ModeSelector extends React.Component {
    ...
    render(){
        ...
        switch(this.state.mode){
            case 'tps':
                image = <img className={'small-img inverted'} title={'Gas mode'} src={gasIcon} onClick={this.gpsSelected.bind(this)}/>;
                para = <p className={'inline'} style={{cursor:'pointer'}} onClick={this.gpsSelected.bind(this)}>{text}</p>;
                break;
            case 'gps':
                image = <img className={'small-img inverted'} title={'Gas-adjusted TPS mode'} src={gasAdjustedIcon} onClick={this.gasAdjustedSelected.bind(this)}/>;
                para = <p className={'inline'} style={{cursor:'pointer'}} onClick={this.gasAdjustedSelected.bind(this)}>{text}</p>;
                break;
            case 'gasAdjustedTPS':
                image = <img className={'small-img inverted'} title={'TPS mode'} src={numberIcon} onClick={this.tpsSelected.bind(this)}/>;
